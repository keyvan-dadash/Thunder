...
TEST(WriterBatonBasicOpreation, CheckThreadSaftyReaderWriter)
{
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        std::thread t1([&](){
            ...
            {
                baton.write_access_start();
                queue.push_back(std::tuple<int, std::thread::id, std::string>(j, std::this_thread::get_id(), gen_random_writebaton(j % 30)));
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        std::thread t1([&](){
            ...
            for (size_t j = 0; j < count_per_thread; j++)
            {
                ...
                result.push_back(std::tuple<int, std::thread::id, std::string, std::thread::id>(j, std::get<1>(s), std::get<2>(s), std::this_thread::get_id()));
                resBaton.write_access_end();
                std::this_thread::sleep_for(std::chrono::microseconds(20));
                baton.read_access_end();
            }
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        std::thread t1([&](){
            ...
            for (size_t j = 0; j < count_per_thread; j++)
            {
                baton.write_access_start();
                queue.push_back(std::tuple<int, std::thread::id, std::string>(j, std::this_thread::get_id(), gen_random_writebaton(j % 30)));
                persistantQueue.push_back(std::tuple<int, std::thread::id, std::string>(j, std::this_thread::get_id(), gen_random_writebaton(j % 30)));
                std::this_thread::sleep_for(std::chrono::microseconds(40));
                total++;
                baton.write_access_end();
            }
