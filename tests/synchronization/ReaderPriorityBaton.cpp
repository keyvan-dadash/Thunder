...
TEST(ReaderBatonBasicCreation, Create) {
  EXPECT_NO_THROW({ thunder::synchronization::ReaderPriorityBaton baton; });
}
    ...
    std::thread t1([&]() {
      ...
      for (size_t j = 0; j < count_per_thread; j++) {
        ...
        queue.push_back(std::tuple<int, std::thread::id, std::string>(
            j, std::this_thread::get_id(), gen_random_readbaton(j % 30)));
...
TEST(ReaderBatonBasicOpreation, CheckReaderFairness) {
  ...
  for (const auto& [key, value] : processedCounter) {
    std::cout << "Thread with id: " << key
              << " processed items with percent of: "
              << (float)value * 100 / (float)totalNumberOfProcessedItem << "%"
              << std::endl;
  }
}