...
TEST(BatonBasicOpreation, CheckReaderFairness)
{
    ...
    int totalNumberOfProcessedItem = count_per_thread * 8;

    for (const auto& [key, value] : processedCounter) {
        std::cout << "Thread with id: " << key << " processed items with percent of: " \
         << (float)value * 100 / (float)totalNumberOfProcessedItem << "%" << std::endl;
    }
}