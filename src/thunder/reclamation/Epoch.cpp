...
Epoch<T>::Epoch() {
  ...
  pthread_key_create(&(this->global_epoch_key_),
                     [](void* param) { free(param); });

  pthread_key_create(&(this->global_recycle_key_),
                     [](void* param) { free(param); });
}
...
void Epoch<T>::registerThisThread() {
  ...
  local_epoch =
      static_cast<LocalEpoch*>(pthread_getspecific(this->global_epoch_key_));
  local_recycle = static_cast<LocalRecycle<T>*>(
      pthread_getspecific(this->global_recycle_key_));
  ...
  if (local_epoch == NULL && local_recycle == NULL) {
    ...
    local_epoch->local_epoch.store(this->currentGlobalEpoch(),
    ...
    std::cout << this->registered_thread_epoch_.back()->local_epoch.load(
                     std::memory_order_relaxed)
              << std::endl;
    std::cout << this->registered_thread_epoch_.front()->local_epoch.load(
                     std::memory_order_relaxed)
              << std::endl;
...
void Epoch<T>::unregistedThisThread() {
  ...
  this->registered_thread_epoch_.erase(
      std::remove_if(this->registered_thread_epoch_.begin(),
                     this->registered_thread_epoch_.end(),
                     [&](LocalEpoch* local) {
                       std::cout << "unregister local " << local->thread_id
                                 << " " << local_epoch->thread_id << " "
                                 << (local->thread_id == local_epoch->thread_id)
                                 << std::endl;
  ...
  auto it = std::find_if(
      ...
      this->registered_thread_recycle_.end(), [&](LocalRecycle<T>* local) {
        std::cout << "unregister recycle " << local->thread_id << " "
                  << local_epoch->thread_id << " "
                  << (local->thread_id == local_epoch->thread_id) << std::endl;
        return local->thread_id == local_epoch->thread_id;
      });
  ...
  this->registered_thread_recycle_.erase(
      it, this->registered_thread_recycle_.end());
...
Epoch<T>::epoch_guard Epoch<T>::enterCriticalPath() {
  ...
  std::cout << "before guard "
            << local_epoch->local_epoch.load(std::memory_order_relaxed)
            << std::endl;
...
void Epoch<T>::retireObj(std::remove_reference_t<T>* t) {
  struct LocalRecycle<T>* local_recycle = static_cast<LocalRecycle<T>*>(
      pthread_getspecific(this->global_recycle_key_));
  ...
  std::cout << this->registered_thread_recycle_.front()->retire_list_.size()
            << std::endl;
}
...
bool Epoch<T>::sync() {
  ...
  // std::cout << local_epoch->local_epoch.load(std::memory_order_relaxed) <<
  // std::endl;
  ...
  for (auto thread_local_epoch : this->registered_thread_epoch_) {
    int64_t thread_epoch =
        thread_local_epoch->local_epoch.load(std::memory_order_relaxed);
    ...
    if (isActive && currentGlobalEpoch != thread_epoch) {
      std::cout << "fuck " << isActive << " " << currentGlobalEpoch << " "
                << thread_epoch << std::endl;
      return false;
    }
  }
  ...
  std::cout << this->registered_thread_recycle_.size() << std::endl;
  std::cout << this->registered_thread_recycle_.back()->retire_list_.size()
            << std::endl;

  this->global_epoch_.store((currentGlobalEpoch + 1) % 3,
                            std::memory_order_relaxed);
