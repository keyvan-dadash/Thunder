...
int ConcurrentQueue<Element, QueueSize>::tryPush(T&& t) {
  ...
  do {
    if (std::abs(head - tail_.load(std::memory_order_relaxed)) >= kQueueSize) {
      return BaseConcurrentQueueStatus::
          CANNOT_INSERT_ELEMENT_QUEUE_SIZE_REACHED_TO_MAX_SIZE;
    }
  } while (!head_.compare_exchange_strong(head, head + 1,
...
int ConcurrentQueue<Element, QueueSize>::pop(Element& element) {
  ...
  } while (!tail_.compare_exchange_strong(tail, tail + 1,
                                          std::memory_order_release));
...
void ConcurrentQueue<Element, QueueSize>::push_atomic(T&& t, int head) {
  ...
  while (true) {
    ...
    if (state.compare_exchange_strong(expectState, CellStates::STORING,
...
void ConcurrentQueue<Element, QueueSize>::pop_atomic(Element& elem, int tail) {
  ...
  while (true) {
    ...
    if (state.compare_exchange_strong(expectState, CellStates::LOADING,
                                      std::memory_order_acquire,
                                      std::memory_order_relaxed)) {
      elem = std::move(*std::launder(
          reinterpret_cast<Element*>(std::addressof(elementArray_[tail]))));
